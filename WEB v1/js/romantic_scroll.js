...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (ringSizeForm && ringSizeInput && ringSizeFeedback) {
        ...
        ringSizeForm.addEventListener('submit', async (event) => {
            ...
            const formData = new FormData(ringSizeForm);
            formData.set('form-name', ringSizeForm.getAttribute('name') || 'ring-size');
            formData.set('ring-size', value);

            const encoded = new URLSearchParams();
            formData.forEach((formValue, key) => {
                if (typeof formValue === 'string') {
                    encoded.append(key, formValue);
                }
            });

            const endpoint = ringSizeForm.getAttribute('action') || '/';
            ...
            const submitToLocalApi = async () => {
                const response = await fetch('/ring-size', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({ ringSize: value }),
                });

                if (!response.ok) {
                    throw new Error(`Local ring-size capture failed with ${response.status}`);
                }
            };
